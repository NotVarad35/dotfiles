...
Item {
  ...
  readonly property var kanjiLabels: ["〇", "一", "二", "三", "四", "五", "六", "七", "八", "九"]
  ...
  function getWorkspaceWidth(ws, activeOverride) {
    ...
    var displayText = root.kanjiLabels[ws.idx] || ws.idx.toString();
    ...
    if (ws.name && ws.name.length > 0) {
      if (root.labelMode === "name") {
        ...
        displayText = (root.kanjiLabels[ws.idx] || ws.idx.toString()) + " " + ws.name.substring(0, characterCount);
      }
    }
          ...
          text: {
            if (groupedContainer.workspaceModel.name && groupedContainer.workspaceModel.name.length > 0) {
              ...
              if (root.labelMode === "index+name") {
                return (root.kanjiLabels[groupedContainer.workspaceModel.idx] || groupedContainer.workspaceModel.idx.toString()) + groupedContainer.workspaceModel.name.substring(0, 1);
              }
            }
            return root.kanjiLabels[groupedContainer.workspaceModel.idx] || groupedContainer.workspaceModel.idx.toString();
          }
