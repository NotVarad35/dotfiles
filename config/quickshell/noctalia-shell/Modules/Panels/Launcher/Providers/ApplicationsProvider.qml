...
Item {
  ...
  function loadApplications() {
    ...
    // Auto-fetch icons for new apps without valid icons
    Qt.callLater(function() {
      ...
      for (var i = 0; i < entries.length; i++) {
        ...
        var exists = false;
        try {
          if (iconName && Quickshell.iconPath && !iconName.startsWith("/")) {
            var p = Quickshell.iconPath(iconName, "");
            exists = !!(p && p !== "");
          }
        } catch (e) {}
        if (!exists && iconName && iconName !== "application-x-executable") {
          var name = app.name || app.id || iconName;
          var id = (app.id || iconName).replace(".desktop", "");
          Logger.d("ApplicationsProvider", "Fetching icon for: " + name);
          Quickshell.execDetached(["fetch-app-icon.sh", id, name]);
          fetched++;
          if (fetched >= 5) break; // limit per reload
        }
      }
    });
  }
  ...
  function resolveAppIcon(app) {
    ...
    // Check if icon exists in the system theme
    try {
    ...
    // Try ThemeIcons for better lookup
    try {
      if (typeof ThemeIcons !== 'undefined') {
        var foundIcon = ThemeIcons.iconForAppId(app.id || app.name, "");
        if (foundIcon && foundIcon !== "")
          return iconName; // keep original name, the path lookup works
      }
    } catch (e) {}

    // Try the app ID directly as icon name
    var appId = app.id || "";
    if (appId && appId !== iconName) {
      try {
        var p = Quickshell.iconPath(appId, "");
        if (p && p !== "")
          return appId;
      } catch (e) {}
    }
