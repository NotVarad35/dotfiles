...
Item {
  ...
  required property var kanjiLabels
          ...
          text: {
            if (workspace.name && workspace.name.length > 0) {
              ...
              if (labelMode === "index+name") {
                ...
                if (isVertical) {
                  return (kanjiLabels[workspace.idx] || workspace.idx.toString()) + workspace.name.substring(0, 1);
                }
                return (kanjiLabels[workspace.idx] || workspace.idx.toString()) + " " + workspace.name.substring(0, characterCount);
              }
            }
            return kanjiLabels[workspace.idx] || workspace.idx.toString();
          }
