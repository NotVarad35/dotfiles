...
Variants {
  ...
  delegate: Item {
    ...
    // DimmerWindow - full-screen background dimmer at WlrLayer.Bottom so windows
    // render between the dimmer and the panels (which stay at Top in MainScreen).
    // Avoids mangowm/dwl skipping Normal layer rendering when Top surfaces go opaque.
    Loader {
      active: parent.windowLoaded && parent.shouldBeActive
      asynchronous: false

      sourceComponent: DimmerWindow {
        screen: modelData
      }
    }
